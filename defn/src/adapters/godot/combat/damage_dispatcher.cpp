...
int DamageDispatcher::apply(godot::ObjectID source_id, AttackTarget *target, int base_damage, DamageDelivery delivery) {
    if (target == nullptr) {
        return 0;
    }
    ...
    const int effective_damage = target->take_damage(resolved_damage, delivery);
