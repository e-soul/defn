...
composition shape, so the mode's quality is entirely a question of whether the escalation `r`, the bounty decay
(`d`, its curve `k` and its floor) and the drift keyframes in `data/endless.json` are right. None of the three can be judged by playing a handful of
...
scons endless seeds=3 decay_curve=0.6,0.8,1.0 bounty_floor=0.05,0.15 out=res://build/endless_income.jsonl
...
| `decay_curve` | shipped `k` | Shape of the income decay, `d^((n-1)^k)`. Below 1 spreads the same reduction over a longer run; `1.0` is the geometric decay every sweep before 2026-09-08 was measured on. |
| `bounty_floor` | shipped | What income never falls below. Substitutes for `decay_curve` rather than adding to it, and is inert below 0.26 -- see the quantization note below. |
...
**Bounty is integer and rounds up, so pick income cells against the staircase.** A kill pays
`ceil(base_bounty * multiplier)` on bounties of 4 to 7, so a multiplier below 0.25 pays 1 energy for every hostile
in the game and two nominally different `decay`/`decay_curve`/`bounty_floor` cells can be the same run. Grime is 41%
of the drift and `4 x 0.25` is exactly 1.0, so the first floor that moves the commonest body is 0.26. A sweep whose
cells straddle no step reads flat and means nothing -- that is an artefact of the instrument, not a finding.

**Read `deployments_total` and `energy_spent` on any income lever, not `waves_reached`.** These knobs move what the
player can afford, not how long they survive, and run length at 5 seeds is mostly seed noise.
