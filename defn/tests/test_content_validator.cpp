...
UiThemeData make_valid_ui_theme() {
    ...
    for (const char *control_icon : {"choice_off", "choice_on", "slider_knob", "slider_knob_disabled", "slider_knob_hot"}) {
        theme.control_icons[control_icon] = {.mark = "res://mark.svg", .color_role = "accent"};
    }
...
DEFN_TEST(content_validator_reports_icon_gaps_and_missing_marks) {
    FakeUnitCatalog units;
    ContentValidationInput input = make_valid_input(units);
    UiThemeData theme = make_valid_ui_theme();
    theme.icons.erase("integrity");
    theme.icons["energy"] = {.mark = "", .color_role = "ghost_energy_color"};
    input.ui_theme = theme;
    input.missing_ui_theme_assets = {"res://assets/ui/hud/gone.svg"};

    const ContentValidationReport report = ContentValidator::validate_loaded_content(input);
    DEFN_CHECK(!report.is_valid());
    DEFN_CHECK(contains_issue(report, "missing required icon 'integrity'"));
    DEFN_CHECK(contains_issue(report, "icon 'energy' is missing a mark"));
...
DEFN_TEST(content_validator_reports_control_icon_gaps_and_missing_marks) {
    FakeUnitCatalog units;
    ContentValidationInput input = make_valid_input(units);
    UiThemeData theme = make_valid_ui_theme();
    theme.control_icons.erase("slider_knob");
    theme.control_icons["choice_on"] = {.mark = "", .color_role = "ghost_choice_color"};
    input.ui_theme = theme;

    const ContentValidationReport report = ContentValidator::validate_loaded_content(input);
    DEFN_CHECK(!report.is_valid());
    DEFN_CHECK(contains_issue(report, "missing required control icon 'slider_knob'"));
    DEFN_CHECK(contains_issue(report, "control icon 'choice_on' is missing a mark"));
    DEFN_CHECK(contains_issue(report, "unknown color role 'ghost_choice_color'"));
}
