...
String find_shoot_frame_path(const UnitConfig &config) {
    for (const auto &[name, animation] : config.animations) {
        if (name == "shoot") {
            return vformat(animation.path_template, 0);
        }
    }
...
Button *DeployCardPresenter::create(const UnitConfig &config, const Callable &pressed_action) {
    ...
    button->set_custom_minimum_size(Vector2(130, 170));
    ...
    const String shoot_frame_path = find_shoot_frame_path(config);
    ...
    name_label->set_text(config.name.capitalize());
    ...
    cost_label->set_text(vformat(String::utf8("\u26A1 %d"), config.cost));
