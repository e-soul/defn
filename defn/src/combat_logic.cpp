...
CombatTargetSelection select_target_from_snapshots(const Vector2 &origin, const CombatConfig &config, AttackTarget *current_target,
    ...
    if (current_target != nullptr) {
        for (const CombatTargetSnapshot &snapshot : targets) {
            if (snapshot.target != current_target || snapshot.dead || snapshot.side == config.side) {
                continue;
            }
    ...
    for (const CombatTargetSnapshot &snapshot : targets) {
        if (snapshot.target == nullptr || snapshot.dead || snapshot.side == config.side) {
            continue;
        }
...
CombatLogicStep advance_combat_logic(const CombatConfig &config, const CombatLogicInput &input) {
    ...
    step.state.target = input.selection.target;
    ...
    if (input.selection.engaged && input.selection.target != nullptr) {
        step.intent.movement = CombatMovementIntent::STOP;
